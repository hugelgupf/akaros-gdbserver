...
void akaros_add_process(unsigned long pid);
int d9c_hit_breakpoint(pid_t pid, uint64_t tid, uint64_t address);
int d9c_add_thread(pid_t pid, uint64_t tid);

static pthread_t debug_read_thread;
static int debug_fd;
static struct d9c_ops debug_ops;
...
int akaros_attach (unsigned long pid) {
  ...
  debug_ops.hit_breakpoint = &d9c_hit_breakpoint;
  debug_ops.add_thread = &d9c_add_thread;
  d9c_init(&debug_ops);

  akaros_add_process(pid);
  if (pthread_create(&debug_read_thread, NULL, d9c_read_thread, &debug_fd)) {
...
void akaros_add_process(unsigned long pid) {
  ...
  dprintf("adding process and thread %P\n", &ptid);
...
int akaros_create_inferior (char *program, char **args) {
  pid_t pid = 0;
  print_func_entry();
  pid = sys_proc_create(program, strlen(program), args, /* envp */ NULL, PROC_DUP_FGRP);
  if (pid < 0) {
    perror("proc_create");
    print_func_exit();
    return -1;
...
static int _delete_thread_callback (struct inferior_list_entry *entry, void *proc)
{
...
void akaros_resume (struct thread_resume *resume_info, size_t n) {
  ...
  struct thread_info *thread = find_thread_ptid(resume_info->thread);
